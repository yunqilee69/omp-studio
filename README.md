...
| 操作 | 位置 |
|---|---|
| 新建会话并发送 | 聊天区底部输入框（没有会话显示时），Enter |
| 搜索 / 找回以前的会话 | 面板头部的放大镜 → 过滤框（标题子串，大小写不敏感；同列表里的文件行就是以前落盘的会话） |
| 置顶 / 取消置顶 | 列表行悬停的 `置顶` 图标，或行右键菜单（按 jsonl 记住顺序） |
| 标记完成（归档） | 列表行悬停的 `完成` 图标，或行右键菜单。归档的行移出列表，收在列表最下面的 `更多 · N 个已归档` 里（点开可召回）；归档实例行会先停掉它的进程 |
| 重命名会话 | 行右键菜单「重命名…」（作用于这一行的实例，不必先切过去） |
| 新建空实例 | 命令面板 `New Instance`，结果出现在列表里 |
| 收起当前会话 / 找会话 | 聊天区顶部 `←` 取消选中回空白页（进程继续跑）；命令面板 `Open Session…` 展开面板并聚焦过滤框 |
...
| 看上下文用量 | 输入框那一排、thinking 按钮右边的圆环：圈里是已用百分比，悬停看已用 / 窗口 token；75% 起黄、90% 起红，omp 还没报过用量的会话不显示 |
