...
对话本身按时间线渲染，不堆卡片：一次工具调用一行（图标 + 动词 + 文件/命令 + `+N -M`，失败追加红色「执行失败」），一段思考一行（`思考 · 持续了 N 秒`）；点任意一行展开结果正文、错误输出与文件入口。秒数是宿主按真实流计时的实测值，从历史 jsonl 回放的会话没有实时事件，就不显示秒数。思考行与工具行左侧内缩一格、压一条竖线：这一串是「这一轮做了什么」，正文仍是齐左的那一层。

**画法（不许退回按窗口裁行）**：一次快照 = 整段对话一次建好（`renderItems(items, full)`，一次插入、一次布局），滚动条覆盖全部内容，往上翻永远有东西，没有占位块、没有「只画尾部 N 行」。离屏行由 `.item { content-visibility: auto }` 交给浏览器跳过布局与绘制，长会话的代价只落在看得见的那几行。流式一帧只更新变化的那一行：正文与思考正文在**同一个节点**里重渲染（`renderItems` 的 upsert 分支 + `patchRow`），不重建行——否则展开的思考块会被重新挂回、正文节点每秒被换掉几十次。折叠块（思考正文、工具结果）在**第一次展开**时才建（`deferredBody`）。同一份 `items` 是 upsert 流：同 key 必须原地替换（`applyItems` → `mergeItems`），按 key 追加会让列表随帧数无限长大、整段重画时画出成百上千个重复行。
