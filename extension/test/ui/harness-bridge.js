...
(() => {
	...
	const logPath = which === "failure" ? "./host-log-failure.json" : which === "plan" ? "./host-log-plan.json" : "./host-log.json";
	const outbound = [];
	let entries = [];
	...
	async function start() {
		...
		for (const entry of entries) {
			...
			// `history` opens a modal; skip unless the host command / hero 打开历史会话 posts it.
			if (entry.message.type === "history") continue;
